...
namespace App.Areas.Identity.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        {
            int totalOrders = await _context.CartDBs.CountAsync();
        ...
        [HttpGet]
        public async Task<IActionResult> ViewOrder(CartDB order)
        {
         
            var userr = await _userManager.GetUserAsync(this.User);

            var orders = _context.CartDBs.Where(x=>x.UserId == userr.Id).OrderByDescending(x=>x.DateCreated);
            return View(await orders.ToListAsync());
        }

        [HttpGet]
        public async Task<IActionResult> OrderAdmin(CartDB order)
        {
         
            return View(await _context.CartDBs.ToListAsync());
        }
        ...
        [HttpGet]
        public async Task<IActionResult> OrderCancel(int? orderid, CartDB orders)
        {
            var order = await _context.CartDBs.FirstOrDefaultAsync(m => m.CartId == orderid);
            if(order == null)
            {
                return NotFound();
            }
            order.Status = "KH Đã hủy";
            _context.Update(order);
            await _context.SaveChangesAsync();
            return RedirectToAction(nameof(ViewOrder), "Manage");


        }
        [HttpGet]
        public async Task<IActionResult> OrderCancelAdmin(int? status,int? orderid, CartDB orders)
        {
            var order = await _context.CartDBs.FirstOrDefaultAsync(m => m.CartId == orderid);
            if(order == null)
            {
            ...
            _context.Update(order);
            await _context.SaveChangesAsync();
            return RedirectToAction(nameof(OrderAdmin), "Manage");
        ...
        public async Task<IActionResult> Delete(int? cartid)
        {
            ...
            var cartdelete = await _context.CartDBs
        ...
        public async Task<IActionResult> Delete(int cartid)
        {
            var cartdelete = await _context.CartDBs.FindAsync(cartid);
            _context.CartDBs.Remove(cartdelete);
            await _context.SaveChangesAsync();
            return RedirectToAction(nameof(ViewOrder));
        }
        [HttpPost]
        public async Task<IActionResult> DeleteAdmin(int cartid)
        {
            var cartdelete = await _context.CartDBs.FindAsync(cartid);
            _context.CartDBs.Remove(cartdelete);
            await _context.SaveChangesAsync();
